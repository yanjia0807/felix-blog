...
export const PostFilter = memo(() => {
  ...
  const doSetFilters = (filters) =>
    dispatch(
      setFilters({
        ...filters,
        publishDateFrom: filters.publishDateFrom && filters.publishDateFrom.toISOString(),
        publishDateTo: filters.publishDateTo && filters.publishDateTo.toISOString(),
      }),
    );
  const doResetFilters = () => dispatch(resetFilters());
